...
\begin{lstlisting}[style=ex]
Trace of \texttt{compose(str, abs)(-3)} -- right to left, so \texttt{abs} runs FIRST:
...
\noindent\textit{Illustration.}
\begin{lstlisting}[style=ex]
Trace of \texttt{unfoldr(step, 3)} where \texttt{step(n) = NOTHING if n==0 else (n, n-1)}:
...
\noindent\textit{Illustration.}
\begin{lstlisting}[style=ex]
Trace of \texttt{getpath} walking the tree \texttt{\{'a': \{'b': \{'c': 42\}\}\}} along
...
\noindent\textit{Illustration.}
\begin{lstlisting}[style=ex]
Trace of a two-step chain, \texttt{bind(bind(4, succ), str)}:
...
\noindent\textit{Illustration.}
\begin{lstlisting}[style=ex]
Trace of a three-step Either chain, each step either passing a value on or attaching a tag:
  start:            Ok(4)
...
\noindent\textit{Illustration.}
\begin{lstlisting}[style=ex]
What \texttt{do} does to the generator above, roughly desugared to nested binds:
...
\noindent\textit{Illustration.}
\begin{lstlisting}[style=ex]
Trace of \texttt{chainl1} folding \texttt{'1+2-3'} strictly LEFT:
  read 1
...
\noindent\textit{Illustration.}
\begin{lstlisting}[style=ex]
Trace of \texttt{both(MaxM, MinM)} folding the pairs \texttt{(3,1)} then \texttt{(5,-2)} --
