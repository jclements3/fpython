...
\end{lstlisting}
\textbf{\texttt{add}} --- (+) as a value, for slots that want a function, not an operator: scanl1, zipWith, foldl.
\begin{lstlisting}[style=ex]
>>> add(2, 3)
5
>>> scanl1(add, [1, 2, 3])
...
\end{lstlisting}
\textbf{\texttt{sub}} --- (-) as a value, same habitat as add.
\begin{lstlisting}[style=ex]
>>> sub(5, 2)
3
...
\end{lstlisting}
\textbf{\texttt{mul}} --- (*) as a value, sibling of add and sub: passable where an operator cannot go. It is Product's combiner in section 16.
\begin{lstlisting}[style=ex]
>>> mul(6, 7)
42
...
\end{lstlisting}
\textbf{\texttt{mconcat}} --- collapses a whole list with a monoid: foldl of the pair's op, seeded by the pair's empty. The empty seed is what makes the empty list a lawful answer instead of an error.
\begin{lstlisting}[style=ex]
>>> mconcat(Sum, [1, 2, 3])
6
>>> mconcat(Sum, [])
...
\end{lstlisting}
\textbf{\texttt{foldMap}} --- map each element into the monoid, then mconcat: the "measure and combine" one-liner. Most monoid uses are foldMap uses -- the elements are rarely already the thing you want to combine.
\begin{lstlisting}[style=ex]
...
\end{lstlisting}
\textbf{\texttt{both}} --- the tuple monoid: pairs two monoids into one, so TWO statistics fall out of ONE pass. Chain it for three or more.
\begin{lstlisting}[style=ex]
...
\end{lstlisting}
\textbf{\texttt{bind}} --- the Maybe monad's >>=: feed x to f, unless x is already None -- then stay None. Failure propagates itself; the if-ladder disappears. The standing caveat of None-as-Nothing applies: a step cannot return None as a SUCCESS.
\begin{lstlisting}[style=ex]
>>> bind(4, succ)
5
...
\end{lstlisting}
\textbf{\texttt{chainM}} --- Kleisli composition (>=>): glue failable steps into one failable pipeline. Unlike compose, the LEFTMOST function runs first -- data flows left to right, reading order.
\begin{lstlisting}[style=ex]
...
\end{lstlisting}
\textbf{\texttt{sequenceM}} --- [Maybe a] -> Maybe [a]: all-or-nothing. Every element present, the list comes back; one None anywhere, the WHOLE answer is None.
\begin{lstlisting}[style=ex]
>>> sequenceM([1, 2, 3])
[1, 2, 3]
...
\end{lstlisting}
\textbf{\texttt{traverseM}} --- map a failable function, then demand every success: sequenceM after map\_. The batch-parse shape -- contrast mapMaybe, which silently DROPS failures; traverseM refuses the whole batch.
\begin{lstlisting}[style=ex]
...
\end{lstlisting}
\textbf{\texttt{bindE}} --- Either's >>=: pass the value onward, or short-circuit carrying the message. The first error wins and survives to the end.
\begin{lstlisting}[style=ex]
...
\end{lstlisting}
\textbf{\texttt{chainE}} --- Kleisli composition for Either: wraps the input in Ok, then threads it through Ok/Err-returning steps. Same shape as chainM, plus the WHY.
\begin{lstlisting}[style=ex]
