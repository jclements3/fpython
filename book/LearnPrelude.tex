...
\end{lstlisting}
\textbf{\texttt{from\_list}} --- build a linked list from a Python list: literally foldr(ListNode, xs, None). foldr's f takes (element, accumulator), and ListNode(val, next) has exactly that shape, so the last element is wrapped first and each earlier one points at the list built so far.
\begin{lstlisting}[style=ex]
