...
\section{foldl}
\noindent\textit{[folds]} --- THE left fold, and the prelude's most important definition. It is exactly this loop: acc = base; for x in xs: acc = f(acc, x); return acc. The combiner takes (ACCUMULATOR, element) -- accumulator first. Argument order of foldl itself: (f, xs, base), with base optional -- omitted, the first element seeds the accumulator (and an empty list is then an error).
\begin{lstlisting}[style=spec]
...
\section{foldMap}
\noindent\textit{[monoids \& monads]} --- map each element into the monoid, then mconcat: the "measure and combine" one-liner. Most monoid uses are foldMap uses -- the elements are rarely already the thing you want to combine.
\begin{lstlisting}[style=spec]
...
\section{traverseM}
\noindent\textit{[monoids \& monads]} --- map a failable function, then demand every success: sequenceM after map\_. The batch-parse shape -- contrast mapMaybe, which silently DROPS failures; traverseM refuses the whole batch.
\begin{lstlisting}[style=spec]
