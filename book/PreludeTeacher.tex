...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
totals = lambda pairs: fromListWith(lambda new, old: new + old, pairs)
\end{lstlisting}
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
restock = lambda inv, item, qty: insertWith(lambda new, old: new + old, item, qty, inv)
\end{lstlisting}
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
roster = lambda rows: fromListWith(
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
by_age_desc = lambda people: sortBy(on(lambda a, b: b - a, snd), people)
\end{lstlisting}
\noindent\textit{Note.} on separates the projection (snd) from the comparison (b - a for descending). This is Haskell's 'comparing' idiom; a per-element key with sortOn would also work here.
