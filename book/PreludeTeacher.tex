...
\title{\Huge\bfseries Prelude Teacher's Edition\\[6pt]\Large A Complete Course in Functional Python\\[14pt]\normalsize seventeen chapters, 201 homework problems, 74 applied labs}
...
style. This Teacher's Edition prints every solution and a teaching note inline; grade against the doctests, then against the style: two or three prelude names and one lambda beat a page of loops.
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{map\_} in \S\ref{def:map-} (p.~\pageref{def:map-}), \texttt{compose} in \S\ref{def:compose} (p.~\pageref{def:compose}).}
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{foldl} in \S\ref{def:foldl} (p.~\pageref{def:foldl}).}
...
mirror = swap
\end{lstlisting}
\noindent\textit{Note.} Recognize-it drill: mirror IS swap. Involution property (twice = identity) is worth saying.
...
score_of = snd
\end{lstlisting}
\noindent\textit{Note.} fst/snd read better than [0]/[1] in pair-heavy code and slot into higher-order positions.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
or_default = fromMaybe
\end{lstlisting}
\noindent\textit{Note.} fromMaybe verbatim. The teaching point is the third test: only None triggers the default, unlike 'x or d' which would destroy the 0.
...
\end{lstlisting}
\noindent\textit{Note.} NOTHING distinguishes omitted from None-as-data; test with 'is'. A None default could not pass the third doctest.
...
make_default = const
\end{lstlisting}
\noindent\textit{Note.} const verbatim -- the constant function factory that fills callback slots.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
by_name = lambda ps: [swap(p) for p in ps]
\end{lstlisting}
\noindent\textit{Note.} swap mapped over the list. O(n); the common slip is reversing the list instead of each pair.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
setting = lambda cfg, key, default: fromMaybe(default, cfg.get(key))
\end{lstlisting}
\noindent\textit{Note.} dict.get is already a None-returning lookup, so fromMaybe finishes it. 'cfg.get(key) or default' would fail the False test.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
invert_book = lambda d: dict(swap(kv) for kv in d.items())
\end{lstlisting}
\noindent\textit{Note.} A dict is a list of pairs wearing a hat: items, swap each, dict. Uniqueness of values is what makes the inversion lossless.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
first_fix = lambda default, log: fromMaybe(
    default, find(isJust, map(snd, log)))
\end{lstlisting}
\noindent\textit{Note.} Reshape then search: bare map keeps the scan lazy, find(isJust, ...) stops at the first real reading, fromMaybe lands the caller's default. The zero test is the None-vs-falsy discipline again -- isJust asks identity against None, never truthiness.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
effective_ids = lambda ps: [fromMaybe(snd(p), fst(p)) for p in ps]
\end{lstlisting}
\noindent\textit{Note.} fromMaybe per element with the pair accessors supplying both sides. 'badge or temp' would fail the blank-badge test.
...
\end{lstlisting}
\noindent\textit{Note.} Two passes, each a one-liner: fromMaybe turns absence into the additive identity; the incomplete count keeps the audit trail honest. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
resolve = lambda key, env, cfg, default: fromMaybe(
    fromMaybe(default, cfg.get(key)), env.get(key))
\end{lstlisting}
\noindent\textit{Note.} fromMaybe NESTS: the inner call resolves cfg-vs-default, and that result becomes the outer default for env. Reading it inside-out is the exercise.
...
\end{lstlisting}
\subsection{\texttt{add}}\label{def:add}
(+) as a value, for slots that want a function, not an operator: scanl1, zipWith, foldl.
\begin{lstlisting}[style=ex]
>>> add(2, 3)
5
>>> scanl1(add, [1, 2, 3])
...
\subsection{\texttt{sub}}\label{def:sub}
(-) as a value, same habitat as add.
\begin{lstlisting}[style=ex]
>>> sub(5, 2)
3
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{zipWith} in \S\ref{def:zipWith} (p.~\pageref{def:zipWith}).}
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
parity_counts = lambda xs: (sum(1 for x in xs if even(x)),
                            sum(1 for x in xs if odd(x)))
\end{lstlisting}
\noindent\textit{Note.} Two generator sums over the parity predicates. One-pass with a fold is a fine alternative; the trap is forgetting that even(-2) and even(0) are True.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
both_divs = lambda a, b: (div(a, b), quot(a, b))
\end{lstlisting}
\noindent\textit{Note.} div floors toward negative infinity (Haskell/Python); quot truncates toward zero (C). Interviews live in the mixed-sign rows of this table.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
char_neighbours = lambda c: (pred(c), succ(c))
\end{lstlisting}
\noindent\textit{Note.} succ/pred dispatch on type: strings step through codepoints, numbers add one. That Enum duality is the whole drill.
...
\end{lstlisting}
\noindent\textit{Note.} Round-trip through the integer square root. The 10**18 tests are exactly where float-based checks start failing.
...
\end{lstlisting}
\noindent\textit{Note.} gcd is always >= 0 (Haskell semantics), so negative inputs need no special casing.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
end_hour = lambda start, duration: mod(start + duration, 24)
\end{lstlisting}
\noindent\textit{Note.} mod's sign follows the divisor, so negative totals land in 0..23 automatically -- the reason floor-mod, not truncating rem, is the clock operator.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
pages = lambda n, k: -div(-n, k)
\end{lstlisting}
\noindent\textit{Note.} ceil(n/k) == -floor(-n/k). The common alternative div(n + k - 1, k) also works; both beat importing math.ceil and converting through floats.
...
\end{lstlisting}
\noindent\textit{Note.} Divide both sides by the gcd -- the definition of lowest terms. O(log min(w, h)) from Euclid.
...
next_together = lcm
\end{lstlisting}
\noindent\textit{Note.} Recognize-it: the question IS lcm. Knowing lcm(a, b) = a * b / gcd(a, b) is the interview follow-up.
...
\end{lstlisting}
\noindent\textit{Note.} signum compresses the three-way comparison; count reads the census. One map, three counts, O(n).
...
def digital_root(n):
    ...
    return n
\end{lstlisting}
\noindent\textit{Note.} Digit-peeling via divmod inside a convergence loop. The slick closed form is 0 if n == 0 else 1 + mod(n - 1, 9) -- casting out nines; mentioning it is a senior signal.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
visible_count = lambda points: sum(1 for (x, y) in points if gcd(x, y) == 1)
\end{lstlisting}
\noindent\textit{Note.} A blocked point's coordinates share a factor g > 1 -- the blocker is (x/g, y/g). gcd's >= 0 semantics make negative quadrants free; gcd(0, 0) == 0 excludes the origin.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
swap_ends = lambda xs: [last(xs)] + init(tail(xs)) + [head(xs)]
\end{lstlisting}
\noindent\textit{Note.} head/last read the ends; init(tail(xs)) is the middle (drop one from each side). O(n) to rebuild. The classic slip is writing xs[1:-1] and forgetting it is empty for length-2 input -- init(tail(...)) handles that for free.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
deal = lambda xs: splitAt(len(xs) // 2, xs)
\end{lstlisting}
\noindent\textit{Note.} splitAt(n, xs) cuts into (xs[:n], xs[n:]) in one call; floor division sends the odd element rightward. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
has_adjacent_dup = lambda xs: any(a == b for a, b in pairwise(xs))
\end{lstlisting}
\noindent\textit{Note.} pairwise turns a list into its consecutive pairs -- exactly the shape any rule about neighbours wants. It yields nothing for length < 2, so any(...) is False there. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
count_distinct = lambda xs: len(nub(xs))
\end{lstlisting}
\noindent\textit{Note.} nub keeps first occurrences (elements must be hashable); its length is the distinct count. O(n). set(xs) would work too but nub is the order-preserving habit.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
lookup_or = lambda book, name, default: fromMaybe(default, lookup(name, book))
\end{lstlisting}
\noindent\textit{Note.} lookup returns the first match or None; fromMaybe supplies the fallback. The pairing of a None-returning search with fromMaybe is the whole Maybe idiom in one line.
...
\end{lstlisting}
\noindent\textit{Note.} transpose is ragged-safe: short rows drop out of later columns instead of truncating every column to the shortest row. null guards the empty sheet. O(cells).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
matches = lambda guess, secret: sum(1 for a, b in zip_(guess, secret) if a == b)
\end{lstlisting}
\noindent\textit{Note.} zip\_ walks both sequences in lockstep, stopping at the shorter; count the agreeing positions. O(n). zip\_ returns a real list, so it also works on strings.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
strip_tag = lambda tag, line: fromMaybe(line, stripPrefix(tag, line))
\end{lstlisting}
\noindent\textit{Note.} stripPrefix returns the remainder after the tag, or None when the tag is absent; fromMaybe falls back to the whole line. An empty tag matches and strips nothing.
...
\end{lstlisting}
\noindent\textit{Note.} isPrefixOf / isSuffixOf work on strings and lists alike. Note isSuffixOf uses the len-based slice, not xs[-len(p):], so an empty suffix correctly matches all. O(n*k).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
pad_to = lambda xs, n, fill: xs + replicate(max(n - len(xs), 0), fill)
\end{lstlisting}
\noindent\textit{Note.} replicate(k, fill) makes k copies; max(..., 0) means an already-long list gets zero padding rather than a negative count. O(n).
...
\end{lstlisting}
\noindent\textit{Note.} nub gives the distinct current names in first-seen order; elem / notElem split them against the previous roster; null reports the empty-history case. O(len(curr)*len(prev)) with lists -- a set(prev) would make membership O(1) if prev were large.
...
\end{lstlisting}
\noindent\textit{Note.} zip3 stitches the parallel arrays into records; drop discards the shipped prefix; enum(..., 1) numbers from 1; unzip splits the kept records back into columns so the weight column can be summed. O(n).
...
Roll your own: two buckets and one conditional append -- (yes if cond(x) else no).append(x). The conditional expression picks the LIST, then the append mutates it; one pass, exactly one predicate call per element.
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{id} in \S\ref{def:id} (p.~\pageref{def:id}), \texttt{partial} in \S\ref{def:partial} (p.~\pageref{def:partial}).}
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
squares_of_evens = lambda xs: map_(lambda x: x * x, filter_(even, xs))
\end{lstlisting}
\noindent\textit{Note.} filter\_ selects, map\_ transforms -- read the pipeline inside-out, filter first. O(n). even is the prelude predicate, ready to hand to filter\_.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
first_over = lambda xs, t: find(lambda x: x > t, xs)
\end{lstlisting}
\noindent\textit{Note.} find is the early exit a fold cannot make: it produces elements lazily and stops at the first match, returning None on failure. O(k) where k is the winner's position.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
split_pass_fail = lambda scores, cutoff: partition(lambda s: s >= cutoff, scores)
\end{lstlisting}
\noindent\textit{Note.} partition returns (keepers, rest) in one pass, calling the predicate exactly once per element -- cheaper and clearer than two filter\_ passes. O(n).
...
\end{lstlisting}
\noindent\textit{Note.} mapMaybe fuses parse-and-filter: the parser answers a value or None, and the Nones are dropped in one pass. lstrip('-') lets a single leading minus through. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
duplicate_each = lambda xs: concatMap(lambda x: [x, x], xs)
\end{lstlisting}
\noindent\textit{Note.} concatMap maps each element to a LITTLE LIST and flattens the results in one step -- the shape for 'each input yields several outputs'. O(n).
...
\end{lstlisting}
\noindent\textit{Note.} catMaybes drops the Nones (it is mapMaybe with the identity); the drop count is the length difference. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
line_totals = lambda prices, quantities: zipWith(mul, prices, quantities)
\end{lstlisting}
\noindent\textit{Note.} zipWith combines two lists elementwise with a function, stopping at the shorter -- the go-to for 'do X to matching positions of two arrays'. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
row_totals = lambda base, tax, tip: zipWith3(lambda b, t, p: b + t + p, base, tax, tip)
\end{lstlisting}
\noindent\textit{Note.} zipWith3 is zipWith for three lists at once -- no intermediate tuples. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
areas = lambda rects: starmap(mul, rects)
\end{lstlisting}
\noindent\textit{Note.} starmap spreads each tuple across a multi-argument function -- map\_ composed with uncurry, so the (w, h) pairs unpack straight into the lambda. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
merge_playlists = lambda playlists: nub(concat(playlists))
\end{lstlisting}
\noindent\textit{Note.} concat flattens the list of lists by one level; nub then drops repeats keeping first occurrence. O(total songs).
...
\end{lstlisting}
\noindent\textit{Note.} cross is the Cartesian product -- every (shirt, pant) combination; filter\_ keeps the affordable ones; a comprehension pulls the names out. O(len(shirts)*len(pants)).
...
\end{lstlisting}
\noindent\textit{Note.} mapMaybe parses each line to a (sign, item) record and drops the blanks; partition splits additions from removals in one pass; comprehensions strip the sign back off. O(n). The common bug is treating a bare 'apple' as malformed rather than an addition.
...
THE left fold, and the prelude's most important definition. It is exactly this loop: acc = base; for x in xs: acc = f(acc, x); return acc. The combiner takes (ACCUMULATOR, element) -- accumulator first. Argument order of foldl itself: (f, xs, base), with base optional -- omitted, the first element seeds the accumulator (and an empty list is then an error).
\begin{lstlisting}[style=ex]
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
sum_squares = lambda xs: foldl(lambda acc, x: acc + x * x, xs, 0)
\end{lstlisting}
\noindent\textit{Note.} One fold, seed 0, combiner acc + x*x; O(n). Common mistake: squaring the accumulator instead of the element.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
longest_word = lambda ws: foldl1(lambda acc, w: w if len(w) > len(acc) else acc, ws)
\end{lstlisting}
\noindent\textit{Note.} foldl1 seeds with the first element; the strict > keeps the incumbent on ties. Using >= would hand ties to the newcomer -- the classic tie-break slip.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
to_pairs = lambda xs: foldr(lambda x, acc: (x, acc), xs, None)
\end{lstlisting}
\noindent\textit{Note.} foldr(f, xs, None) with f = make-a-pair; the seed None is the empty-list arm. Trying this with foldl builds the chain backwards.
...
\end{lstlisting}
\noindent\textit{Note.} product is the numeric fold seeded with 1, so the n = 0 case falls out for free; O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
powerset_strings = lambda s: map_("".join, subsequences(s))
\end{lstlisting}
\noindent\textit{Note.} subsequences gives lists of characters; one map\_ of join finishes. 2\^{}n results -- say the bound before calling it on anything long.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
balance_audit = lambda start, txns: foldl(
    lambda st, x: (st[0] + x, st[1] or st[0] + x < 0), txns, (start, False))
\end{lstlisting}
\noindent\textit{Note.} Tuple accumulator (balance, flag); the 'or' latches the flag once tripped. O(n). Common mistake: checking only the final balance.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
ways_to_roll = lambda n, target: len([r for r in replicateM(n, range(1, 7)) if sum(r) == target])
\end{lstlisting}
\noindent\textit{Note.} replicateM(n, faces) is every ordered roll; filter and count. 6\^{}n outcomes -- state the bound. The DP version exists, but at n <= 6 enumeration is simpler and safer.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
rle_decode = lambda pairs: foldl(lambda acc, p: acc + p[0] * p[1], pairs, '')
\end{lstlisting}
\noindent\textit{Note.} A fold whose accumulator is the output string; char * count expands each run. The inverse of the g5 rle\_encode problem -- round-trip them as a self-check.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
apply_edits = lambda text, edits: foldl(lambda t, e: t.replace(e[0], e[1]), edits, text)
\end{lstlisting}
\noindent\textit{Note.} The accumulator is the evolving TEXT and the list folded over is the edits -- a fold where state is a document, not a number. Order sensitivity is the point.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
exact_change = lambda coins, target: any(sum(s) == target for s in subsequences(coins))
\end{lstlisting}
\noindent\textit{Note.} The powerset as a brute-force license: 2\^{}n subsets at n <= 20 is about a million cheap checks. any() stops at the first witness.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def min_team(required, candidates):
    covers = [s for s in subsequences(candidates) if set(required) <= set(concat(s))]
    return min(map_(len, covers), default=-1)
\end{lstlisting}
\noindent\textit{Note.} Set cover is NP-hard in general; n <= 12 licenses the 2\^{}n sweep. concat flattens a group's skills; set inclusion checks coverage; min with default handles impossibility.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def min_partition_diff(xs):
    total = sum(xs)
    return min(abs(total - 2 * sum(s)) for s in subsequences(xs))
\end{lstlisting}
\noindent\textit{Note.} The classic balanced-partition interview problem, honestly brute-forced: one algebraic observation turns it into a powerset minimum. 2\^{}n at n <= 20; the DP version is the follow-up conversation.
...
def next_greater(xs):
    ...
    return res
\end{lstlisting}
\noindent\textit{Note.} The monotonic stack holds indices still waiting for their answer; each index is pushed once and popped at most once, so the fold is O(n). 'Next greater' is the recognition ladder's textbook STACK trigger -- same skeleton as matching pairs of brackets.
...
a fold that KEEPS ITS HISTORY: returns every intermediate accumulator, starting with the seed, so the result has len(xs)+1 elements and its last element equals the foldl. Argument order (f, z, xs) -- seed in the middle, Haskell's order, unlike foldl's trailing base. Prefix sums are the canonical scan.
\begin{lstlisting}[style=ex]
>>> scanl(add, 0, [3, 1, 4])
[0, 3, 4, 8]
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
running_max = lambda xs: scanl1(max, xs)
\end{lstlisting}
\noindent\textit{Note.} scanl1 seeds with the first element and keeps every intermediate; max is the combiner. O(n). This scan IS the best-so-far column of many DP tables.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
account_history = lambda start, txns: scanl(add, start, txns)
\end{lstlisting}
\noindent\textit{Note.} scanl's seed appears as the first output, which models the opening balance exactly; the n+1 length is the contract, not a bug.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
suffix_sums = lambda xs: scanr(add, 0, xs)
\end{lstlisting}
\noindent\textit{Note.} scanr folds from the right and keeps every suffix's result; the whole-list answer lands first. Mirror image of prefix sums.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
best_remaining = lambda prices: scanr1(min, prices)
\end{lstlisting}
\noindent\textit{Note.} A right-to-left running minimum: scanr1(min). Pairing it with a left scan is the shape of many buy/sell problems.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
running_product = lambda xs: list(accumulate(xs, mul))
\end{lstlisting}
\noindent\textit{Note.} accumulate defaults to addition; hand it * for products. It yields lazily, so list() materialises -- forgetting that is the usual slip.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def new_records(temps):
    if temps == []:
        return 0
    return 1 + len([1 for a, b in pairwise(scanl1(max, temps)) if b > a])
\end{lstlisting}
\noindent\textit{Note.} scanl1(max) turns history into a staircase; each strict step up is a record, plus one for day zero. Watch the tie case: equalling the record is not breaking it.
...
\end{lstlisting}
\noindent\textit{Note.} One O(n) scan converts every window question into two array reads. The seed 0 makes pre[i] the sum of the first i elements, so half-open (i, j) needs no off-by-one fixups.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def fuel_report(start, deltas):
    history = scanl(add, start, deltas)
    return (min(history), min(history) >= 0)
\end{lstlisting}
\noindent\textit{Note.} The scan is the whole flight recorder; min answers both questions. A bare fold would need a compound accumulator to remember the low-water mark.
...
\end{lstlisting}
\noindent\textit{Note.} scanl1(max) is the peak line; zipWith subtracts the actual price under it. The mirror of best-time-to-sell: there you chase rises off the running MIN.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
peak_concurrency = lambda events: max(scanl(add, 0, events))
\end{lstlisting}
\noindent\textit{Note.} The seed 0 doubles as the empty-log answer since scanl always emits it. This is the sweep-line pattern in miniature: deltas, running level, extremum.
...
\end{lstlisting}
\noindent\textit{Note.} LeetCode 42 without a single index: left-max scan, right-max scan, pointwise min(l, r) - h. Each term is >= 0 because each scan includes the bar itself.
...
\end{lstlisting}
\noindent\textit{Note.} LeetCode 1014: split the pair formula, scan the left part's running max, zip it one position behind the right part. init/tail do the i < j offset with no index fiddling.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
multiples = lambda k, n: take(n, count(k, k))
\end{lstlisting}
\noindent\textit{Note.} count(k, k) is the infinite stream k, 2k, 3k, ...; take stops it. O(n). Common mistake: reaching for range arithmetic when the stream says it directly.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
banner = lambda ch, n: "".join(repeat(ch, n))
\end{lstlisting}
\noindent\textit{Note.} repeat(x, n) is the finite form; join materialises it. O(n). Same result as ch * n -- the point is knowing repeat's two modes.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
powers = lambda b, n: take(n, iterate(lambda x: x * b, 1))
\end{lstlisting}
\noindent\textit{Note.} iterate(f, 1) is 1, b, b\^{}2, ... forever; take cuts it. O(n) multiplications.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
countdown = lambda n: unfoldr(lambda m: None if m == 0 else (m, m - 1), n)
\end{lstlisting}
\noindent\textit{Note.} The two-outcome contract in miniature: emit (m, m-1) or stop with None. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
splice = lambda a, b, c: list(chain(a, b, c))
\end{lstlisting}
\noindent\textit{Note.} chain is lazy concatenation; list() drains it. O(total length).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
deal = lambda players, cards: zip_(cycle(players), cards)
\end{lstlisting}
\noindent\textit{Note.} cycle makes the rotation infinite; zip\_ stops at the shorter side -- the cards. O(cards). Common mistake: index arithmetic with \% when cycle already says it.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
days_to_reach = lambda start, goal: len(
    takeWhile(lambda x: x < goal, iterate(lambda x: 2 * x, start)))
\end{lstlisting}
\noindent\textit{Note.} The stream is start, 2*start, ...; the answer is how many terms stay under goal. O(log(goal/start)). Watch the >= boundary: day counting is off by one if you use <=.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
bits = lambda n: list(reversed(unfoldr(lambda m: None if m == 0 else (m % 2, m // 2), n)))
\end{lstlisting}
\noindent\textit{Note.} unfoldr naturally emits LSB first (m \% 2 peels the low bit); one reversal fixes the display order. O(log n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
chain_of_command = lambda boss, name: unfoldr(
    lambda n: None if n is None else (n, boss[n]), name)
\end{lstlisting}
\noindent\textit{Note.} A parent-chain walk is an unfold: emit the person, seed becomes the manager, None stops. O(depth). The same shape reconstructs paths in Dijkstra.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
sync = lambda a, da, b, db: find(lambda x: x >= b and (x - b) % db == 0, count(a, da))
\end{lstlisting}
\noindent\textit{Note.} find is the only consumer that can search an infinite count stream: it stops at the first hit. The membership test for B's schedule is arithmetic, not a second stream.
...
\end{lstlisting}
\noindent\textit{Note.} Two unfolds stacked: unfoldr peels one term into runs (find locates where the run ends, fromMaybe handles the final run), and iterate unfolds the sequence of terms. O(n * term length). Common mistake: forgetting the last run when no differing digit exists.
...
\end{lstlisting}
\noindent\textit{Note.} unfoldr emits every term before 1, so trajectory length is len + 1. max with key= keeps the FIRST winner on ties, and range ascends, so ties resolve to the smaller start for free. Interview follow-up: memoize lengths across starts with memo.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
leading_positive = lambda xs: span(lambda x: x > 0, xs)[0]
\end{lstlisting}
\noindent\textit{Note.} span returns (prefix, rest) in one pass; take the first component. O(prefix length).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
peek = lambda k, xs: take(k, xs)
\end{lstlisting}
\noindent\textit{Note.} take is the safe window onto an infinite stream; islice underlies it. O(k).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
skip_header = lambda lines: dropWhile(lambda s: s.startswith('#'), lines)
\end{lstlisting}
\noindent\textit{Note.} dropWhile discards while true, then keeps the rest untested -- note '\# c' survives. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
prefixes = lambda xs: inits(xs)
\end{lstlisting}
\noindent\textit{Note.} inits gives len(xs)+1 prefixes, empty first. O(n\^{}2) total size.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
paginate = lambda k, xs: chunksOf(k, xs)
\end{lstlisting}
\noindent\textit{Note.} chunksOf is pure slicing, so it works on lists and strings alike. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
read_int = lambda s: int("".join(span(str.isdigit, s)[0]) or "0")
\end{lstlisting}
\noindent\textit{Note.} span(str.isdigit, s)[0] is the digit prefix as a char list; join and int it, with 'or 0' for the empty case. This is exactly how a lexer peels one token. O(len).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def split_email(lines):
    head, rest = break_(lambda s: s == '', lines)
    return (head, rest[1:])
\end{lstlisting}
\noindent\textit{Note.} break\_ splits at the first line where the predicate holds -- the blank -- putting it at the front of rest, so rest[1:] drops the separator. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
read_record = lambda xs: list(takeuntil(lambda x: x == -1, xs))
\end{lstlisting}
\noindent\textit{Note.} takeuntil INCLUDES the first hit, unlike takewhile which would stop just before -1. O(record length). Common mistake: using takewhile and losing the sentinel.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
warm_streak = lambda temps, floor: len(span(lambda t: t >= floor, temps)[0])
\end{lstlisting}
\noindent\textit{Note.} span isolates the leading run, len measures it -- one pass, no manual counter. O(streak).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
substrings = lambda s: concatMap(
    lambda suf: map_(lambda p: "".join(p), inits(list(suf))[1:]),
    takeWhile(lambda suf: suf != "", tails(s)))
\end{lstlisting}
\noindent\textit{Note.} tails gives every suffix (dropping the empty tail); each suffix's inits[1:] are its non-empty prefixes -- together, every substring. O(n\^{}2) substrings, the true size.
...
\end{lstlisting}
\noindent\textit{Note.} span(equal-to-head) is exactly one run; unfoldr repeats the peel until empty. This is how group is built. O(n). Common mistake: comparing to the previous element instead of the run's head.
...
\end{lstlisting}
\noindent\textit{Note.} unfoldr peels one line per step; takeWhile(fits, count(1)) finds the largest word count that fits, and last takes it. O(total characters). Every word fitting alone guarantees k >= 1, so the unfold always makes progress.

\noindent{\small\itshape Rolled later: \texttt{words} in \S\ref{def:words} (p.~\pageref{def:words}), \texttt{unwords} in \S\ref{def:unwords} (p.~\pageref{def:unwords}).}
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{unwords} in \S\ref{def:unwords} (p.~\pageref{def:unwords}), \texttt{on} in \S\ref{def:on} (p.~\pageref{def:on}).}
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
sort_by_size = lambda ws: sortOn(lambda w: (len(w), w), ws)
\end{lstlisting}
\noindent\textit{Note.} Tuple keys sort level by level; O(n log n). Common mistake: two separate sorts, which works only because Python's sort is stable but hides the intent.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
closest_to_zero = lambda xs: minOn(abs, xs)
\end{lstlisting}
\noindent\textit{Note.} minOn is O(n) and keeps the first winner on ties -- no sort needed. Sorting first would cost O(n log n) and lose the arrival tie-break.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
count_val = lambda xs, x: bisect_right(xs, x) - bisect_left(xs, x)
\end{lstlisting}
\noindent\textit{Note.} bisect\_left finds the first slot >= x, bisect\_right the first > x; the gap between them IS the run of equals. Two O(log n) probes, zero scanning.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
normalize_spaces = lambda s: unwords(words(s))
\end{lstlisting}
\noindent\textit{Note.} words splits on any whitespace runs; unwords rejoins with single spaces. The round trip IS the normalizer -- no regex required.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
merge3 = lambda a, b, c: merge(merge(a, b), c)
\end{lstlisting}
\noindent\textit{Note.} merge is associative, so nesting two calls gives a three-way merge in O(n) total. This chaining is the seed of k-way merging.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
leaderboard = lambda es: map_(fst, sortOn(lambda e: (-e[1], e[0]), es))
\end{lstlisting}
\noindent\textit{Note.} The (-score, name) tuple key gets descending score and ascending name in one sort; reverse=True could not, since it would flip the names too.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
shipping_class = lambda w: ["envelope", "small", "medium", "freight"][bisect_right([1, 5, 20], w)]
\end{lstlisting}
\noindent\textit{Note.} Every threshold ladder is a sorted list in disguise; bisect\_right sends boundary values upward. Mistake to know: bisect\_left would send them down.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
sort_versions = lambda vs: sortOn(lambda v: [int(p) for p in v.split(".")], vs)
\end{lstlisting}
\noindent\textit{Note.} The key turns each version into a list of ints, and Python compares lists elementwise with shorter-prefix-first -- exactly version semantics. Sorting the raw strings is the classic bug ('1.10' < '1.2' lexically).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
sort_report = lambda text: unlines(
    sortOn(lambda l: (-int(words(l)[1]), words(l)[0]), lines(text)))
\end{lstlisting}
\noindent\textit{Note.} lines/unlines bracket the pipeline; the tuple key does both orderings. unlines terminates every line, hence the trailing newline in the contract.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
merge_unique = lambda a, b: nub(merge(a, b))
\end{lstlisting}
\noindent\textit{Note.} merge keeps the result sorted in O(n+m); nub then removes adjacent (and any) duplicates keeping first occurrences. Sorting a concatenation would also work but pays O(n log n) for order the inputs already had.
...
\end{lstlisting}
\noindent\textit{Note.} merge gives the combined order in O(n+m); the median reads off the middle. The famous interview escalation is the O(log) binary-partition version -- know that it exists and say so.
...
\end{lstlisting}
\noindent\textit{Note.} Sorting makes the lexicographic extremes the only witnesses needed; zip\_ + takeWhile walks the agreement. O(S log n) for total char count S. Common mistake: comparing only adjacent pairs and merging results.
...
The order gotcha, faithfully Haskell's: because f receives (new, old), grouping with a bare concat REVERSES each group -- the classic Data.Map surprise. Combine as flip(add) (old + new) to keep arrival order, or use plain add where order cannot matter (counts).
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{group} in \S\ref{def:group} (p.~\pageref{def:group}), \texttt{flip} in \S\ref{def:flip} (p.~\pageref{def:flip}).}
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{partial} in \S\ref{def:partial} (p.~\pageref{def:partial}).}
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
vowel_census = lambda s: sorted(
    Counter(filter_(lambda ch: ch in "aeiou", s.lower())).items())
\end{lstlisting}
\noindent\textit{Note.} filter\_ narrows to vowels, Counter folds the counts, sorted makes the output deterministic. Reading Counter with [] would autovivify zeros -- returning .items() sidesteps that entirely.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
totals = lambda pairs: fromListWith(add, pairs)
\end{lstlisting}
\noindent\textit{Note.} Addition is commutative, so the f(new, old) order cannot bite here. This is THE dict-building fold at its simplest: pairs in, totals out, O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
longest_run = lambda s: max(map_(len, group(s)), default=0)
\end{lstlisting}
\noindent\textit{Note.} group returns bare runs ([[a]]), so map\_ len measures them directly; default=0 covers the empty string. One pass, O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
restock = lambda inv, item, qty: insertWith(add, item, qty, inv)
\end{lstlisting}
\noindent\textit{Note.} insertWith copies the dict first -- that copy IS the persistence -- then combines f(new, old) on collision. Mutating inv directly is the mistake this drill exists to catch.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
can_build = lambda word, tiles: bag_sub(Counter(word), Counter(tiles))
\end{lstlisting}
\noindent\textit{Note.} Two Counter folds and an inclusion test; O(n + m). A set-based check is the classic wrong answer -- it ignores multiplicity.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
high_water = lambda a, b: unionWith(max, a, b)
\end{lstlisting}
\noindent\textit{Note.} unionWith(max) is exactly bag\_union generalized to any dict; a's keys keep their positions, b's new keys append. Swapping in addition would instead give a summing merge -- same shape, different monoid.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
sessions = lambda ts: groupBy(lambda start, t: t - start <= 30, ts)
\end{lstlisting}
\noindent\textit{Note.} groupBy hands eq the run's FIRST element and each newcomer -- precisely the anchored-window rule. A neighbor-gap rule would need pairwise instead; confusing the two is the point of this exercise.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
roster = lambda rows: fromListWith(flip(add), [(d, [e]) for d, e in rows])
\end{lstlisting}
\noindent\textit{Note.} Wrap each employee as a one-element list, then combine old + new to keep arrival order -- f receives the NEW value first (Haskell's order), so a plain (+) reverses every roster. That reversal is the classic gotcha.

\noindent{\small\itshape Rolled later: \texttt{flip} in \S\ref{def:flip} (p.~\pageref{def:flip}).}
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
shopping_list = lambda need, have: sorted(
    bag_diff(Counter(need), Counter(have)).items())
\end{lstlisting}
\noindent\textit{Note.} bag\_diff subtracts with clipping at zero, so surplus pantry items vanish rather than going negative; sorted makes the output order deterministic.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
rollup = lambda reports: foldl(lambda acc, d: unionWith(add, acc, d), reports, {})
\end{lstlisting}
\noindent\textit{Note.} unionWith(+) merges two reports; foldl extends that to any number, seeded with the empty dict (the monoid's identity). Key order: first server to mention a page owns its position.
...
\end{lstlisting}
\noindent\textit{Note.} Counter tallies in O(n); maxOn over the sorted keys picks a deterministic top; the strict > len/2 test rejects mere pluralities. Interview escalation: Boyer-Moore voting does it in O(1) space -- name it.
...
\end{lstlisting}
\noindent\textit{Note.} Fold one: totals keyed by the (city, customer) PAIR. Fold two: regroup by city, keeping arrival order with old + new. minOn over (-total, name) gets highest-spend-then-alphabetical in one key. O(n) folds plus a per-city scan.
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{leaves} in \S\ref{def:leaves} (p.~\pageref{def:leaves}).}
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{paths} in \S\ref{def:paths} (p.~\pageref{def:paths}), \texttt{setpath} in \S\ref{def:setpath} (p.~\pageref{def:setpath}), \texttt{on} in \S\ref{def:on} (p.~\pageref{def:on}).}
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{setpath} in \S\ref{def:setpath} (p.~\pageref{def:setpath}), \texttt{partial} in \S\ref{def:partial} (p.~\pageref{def:partial}), \texttt{compose} in \S\ref{def:compose} (p.~\pageref{def:compose}).}
...
def set_all(pairs):
    ...
    return t
\end{lstlisting}
\noindent\textit{Note.} One setpath per pair; autovivification builds every interior node. O(total path length). Common mistake: forgetting that setpath at an interior key clobbers the subtree below it -- here that is the specified overwrite behaviour.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
get_setting = lambda cfg, dotted, default: getpath(cfg, dotted.split('.'), default)
\end{lstlisting}
\noindent\textit{Note.} split('.') turns the dotted string into a key path; getpath does the safe descent. Common mistake: chained cfg[a][b] indexing, which raises on misses and autovivifies on defaultdicts.
...
leaf_values = leaves
\end{lstlisting}
\noindent\textit{Note.} This IS the prelude's leaves -- the second column of paths. Like flatten == concat, recognising an existing tool is the whole exercise.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def sales_table(pairs):
    t = Tree(2, int)
    for city, product in pairs:
        t[city][product] += 1
    return t
\end{lstlisting}
\noindent\textit{Note.} Tree(2, int) autovivifies the city level and hands the product level an int leaf factory, so the whole body is one +=. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
key_paths = lambda t: ['/'.join(p) for p, _ in paths(t)]
\end{lstlisting}
\noindent\textit{Note.} paths hands back (keypath, leaf) rows; keep the first column and join. O(total keys).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def count_with_prefix(numbers, prefix):
    t = ITree()
    for n in numbers:
...
\end{lstlisting}
\noindent\textit{Note.} The '\$' terminator is what lets '12' be counted under prefix '12' even though '123' continues past it. getpath (never indexing) keeps the miss case from planting a ghost branch. O(total digits) to build, O(subtree) to count.
...
def deep_merge(a, b):
    ...
    return out
\end{lstlisting}
\noindent\textit{Note.} unionWith's shape, recursing when both sides are dicts -- dict(a) at every level is the persistence (compare insertWith). Common mistake: out[k].update(v), which mutates a's subtree in place.
...
\end{lstlisting}
\noindent\textit{Note.} A file path resolves to an int, and paths() on a non-dict yields one ([], leaf) row, so sum(leaves(...)) covers files and folders alike. The == {} guard matters: paths treats an empty dict as a LEAF, so summing the miss default would try 0 + {}.
...
def word_index(words):
    ...
    return t
\end{lstlisting}
\noindent\textit{Note.} Tree(2, list): two autovivified levels ending in list leaves, so grouping is a bare append. The fromListWith alternative needs nested dict surgery -- fixed-depth Tree is the cleaner fit here.
...
\end{lstlisting}
\noindent\textit{Note.} paths flattens, join+str renders, sorted makes the output deterministic. The empty guard exists because paths({}) reports the empty dict itself as a leaf row -- a corner of the paths contract worth remembering.
...
def common_prefix(words):
    ...
    return out
\end{lstlisting}
\noindent\textit{Note.} The walk stops at the first branch point OR the first '\$' -- the terminator is what stops 'car'/'card' at 'car'. Membership ('\$' in node) is safe on an ITree: 'in' never autovivifies, only indexing does.
...
def tree_lines(dirs):
    ...
    return emit(t, 0)
\end{lstlisting}
\noindent\textit{Note.} The build needs no setpath because there is no leaf value -- indexing autovivifies the whole branch (the deepest\_directory move). paths() cannot render interior nodes, so the emitter is a hand recursion over items(); that is the intended lesson.
...
def rotate_left(xs, k):
    ...
    return list(d)
\end{lstlisting}
\noindent\textit{Note.} Each rotation is popleft + append, both O(1); k \% len caps the work. Common mistake: forgetting the empty-list guard before the modulo.
...
def last_k(xs, k):
    ...
    return list(d)
\end{lstlisting}
\noindent\textit{Note.} Append at the back, evict from the front: the bounded-buffer idiom, O(1) per event. A plain list doing pop(0) here would be O(n) per eviction.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def same_group(n, pairs, queries):
    f, union = dsu(n)
    for a, b in pairs:
        union(a, b)
    return [f(a) == f(b) for a, b in queries]
\end{lstlisting}
\noindent\textit{Note.} Union everything, then each query is two finds. Near O(1) per operation with path halving. Naming the local 'f' avoids shadowing the prelude's find.
...
\end{lstlisting}
\noindent\textit{Note.} n pushes + n pops, each O(log n): O(n log n) total. Duplicates survive -- a heap is a bag, not a set.
...
\end{lstlisting}
\noindent\textit{Note.} O(n log n) build, O(k log n) drain -- beats fully sorting when k is small. min(k, len) is the guard the last doctest checks.
...
def survivor(names, m):
    ...
    return d.popleft()
\end{lstlisting}
\noindent\textit{Note.} The deque IS the circle: rotation moves the count, popleft eliminates. O(n * m). Common mistake: rotating m times instead of m - 1.
...
def merge_all(lists):
    ...
    return out
\end{lstlisting}
\noindent\textit{Note.} The heap holds at most one entry per list, so each of the N total readings costs O(log k): O(N log k), the multiway-merge bound. This is the prelude's two-list merge generalised by a frontier.
...
\end{lstlisting}
\noindent\textit{Note.} Count roots with a set comprehension over find -- roots are arbitrary but distinct per component. The final full pass of finds also flattens every chain.
...
\end{lstlisting}
\noindent\textit{Note.} find's laziness meets union's side effect: edges are merged one at a time until one reports False, and nothing after it is touched. Keep the dsu locals named f/union so the prelude's find stays visible.
...
def kth_largest_stream(xs, k):
    ...
    return out
\end{lstlisting}
\noindent\textit{Note.} The bounded min-heap holds the k largest; evicting the MIN keeps exactly them, and h[0] reads the k-th largest in O(1). O(n log k). Common mistake: a max-heap, which cannot evict its smallest.
...
def window_max(xs, k):
    ...
    return out
\end{lstlisting}
\noindent\textit{Note.} The monotonic deque: every index enters once and leaves once (back-pop when beaten, front-pop when expired), so the nested while is still O(n) amortised. The pop/push- back peeks are the standard workaround for a peekless deque.
...
\end{lstlisting}
\noindent\textit{Note.} Greedy-by-weight is optimal because a cheapest edge crossing any cut belongs to some minimum spanning tree; union==False is the cycle filter. O(E log E) for the sort; the dsu work is effectively linear. merges == n-1 doubles as the connectivity test.
...
build a linked list from a Python list: literally foldr(ListNode, xs, None). foldr's f takes (element, accumulator), and ListNode(val, nxt) has exactly that shape, so the last element is wrapped first and each earlier one points at the list built so far.
\begin{lstlisting}[style=ex]
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{partial} in \S\ref{def:partial} (p.~\pageref{def:partial}), \texttt{compose} in \S\ref{def:compose} (p.~\pageref{def:compose}).}
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
tree_sum = lambda t: tfold(lambda v, l, r: v + l + r, 0, t)
\end{lstlisting}
\noindent\textit{Note.} The catamorphism with f = plus and z = 0. O(n). Common slip: forgetting z covers BOTH missing children and the empty tree.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
tree_max = lambda t: tfold(lambda v, l, r: max(v, l, r), float('-inf'), t)
\end{lstlisting}
\noindent\textit{Note.} f = three-way max, z = -infinity so absent children never win. O(n). The non-empty contract is what keeps -inf from leaking out.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
count_leaves = lambda t: tfold(lambda v, l, r: (l + r) or 1, 0, t)
\end{lstlisting}
\noindent\textit{Note.} (l + r) or 1: internal nodes pass the children's total through; a leaf's 0 total falls back to 1. O(n). The 'or' trick replaces an explicit is-leaf test tfold cannot make.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
llen = lambda node: len(to_list(node))
\end{lstlisting}
\noindent\textit{Note.} to\_list then len: O(n) either way, and the list form is printable and testable. from\_list([]) is None -- the empty list IS the null pointer.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
traversals = lambda t: (inorder(t), preorder(t), postorder(t))
\end{lstlisting}
\noindent\textit{Note.} inorder = left,node,right; preorder = node first (serialise/copy order); postorder = node last (delete/evaluate order). A left chain makes pre and post read as reverses.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
kth_from_end = lambda node, k: to_list(node)[-k]
\end{lstlisting}
\noindent\textit{Note.} Materialise once, index from the end: O(n) time either way. The two-pointer gap trick saves memory, not time -- say that trade aloud if asked.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def is_palindrome_list(node):
    xs = to_list(node)
    return xs == xs[::-1]
\end{lstlisting}
\noindent\textit{Note.} Convert, compare with the reversed slice: O(n) time, O(n) space. The O(1)-space version (reverse\_list on the back half from middle) is the follow-up interviewers like.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
dedup = lambda node: from_list(nub(to_list(node)))
\end{lstlisting}
\noindent\textit{Note.} Round-trip through list-land: to\_list, nub (first occurrence wins -- on sorted input that equals adjacent-dedup), from\_list. O(n). Pointer surgery is the O(1)-extra-space alternative.
...
\end{lstlisting}
\noindent\textit{Note.} inorder + pairwise + all: three prelude names replace the classic min/max-bounds recursion. Strict < also rejects duplicates. O(n). The bounds recursion is the O(1)-extra-space follow-up.
...
def is_symmetric(t):
    ...
    return shape == mirror
\end{lstlisting}
\noindent\textit{Note.} tfold with (v, l, r) serialises the exact structure as nested tuples; swapping to (v, r, l) builds the mirror's serialisation. Equal tuples == symmetric tree. O(n). Plain inorder comparison is NOT enough -- it loses structure.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
merge_k = lambda feeds: foldl(merge_lists, feeds, None)
\end{lstlisting}
\noindent\textit{Note.} foldl of the two-way merge, seeded with the empty list. O(k*N) total; the heap-of-heads version is O(N log k) -- name it, then justify the simpler fold at interview scale.
...
\end{lstlisting}
\noindent\textit{Note.} The compound-accumulator trick: fold up (height, ok) pairs so balance is checked at every node in one O(n) pass. The classic mistake is checking only the root's height difference -- the fourth test catches exactly that.
...
How it works: inversion of control. The skeleton owns the two pointers -- it grows hi one element per step (calling push), then shrinks lo while your valid() reports the window broken (calling shed per evicted element) -- and it tracks the best length ever seen legal. You own only the STATE, held in closures over variables sitting next to the call.


Designing the closures is answering one question: "what makes a window ILLEGAL, and what must I track to know?" Distinct characters -> a duplicate count. A budget -> a running sum. At most k of something -> a counter. push and shed are that state's increment and decrement; valid is the legality test.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
degree = lambda r, c, R, C: len(list(neighbors4(r, c, R, C)))
\end{lstlisting}
\noindent\textit{Note.} neighbors4 is a generator, so list() it before len(). Corners have 2, edges 3, interior 4 -- and a 1x1 grid has none.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
diag_neighbors = lambda r, c, R, C: sorted(
    set(neighbors8(r, c, R, C)) - set(neighbors4(r, c, R, C)))
\end{lstlisting}
\noindent\textit{Note.} Set difference of the 8-neighbourhood and the 4-neighbourhood, sorted for a deterministic answer. A 1-row grid has no diagonals at all.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
live_around = lambda grid, r, c: sum(
    grid[nr][nc] == '1' for nr, nc in neighbors8(r, c, len(grid), len(grid[0])))
\end{lstlisting}
\noindent\textit{Note.} sum over a generator of booleans counts the Trues; bounds live inside neighbors8, so no edge special-cases. This is the inner loop of every Life step.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def longest_positive(xs):
    bad = [0]
    def add(x): bad[0] += x <= 0
    def rem(x): bad[0] -= x <= 0
    return longest_window(xs, lambda: bad[0] == 0, add, rem)
\end{lstlisting}
\noindent\textit{Note.} State = a count of offenders in the window; valid means zero offenders. The one-element list is the standard closure-mutable-int idiom. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
border_count = lambda R, C: len(
    [1 for r in range(R) for c in range(C) if len(list(neighbors4(r, c, R, C))) < 4])
\end{lstlisting}
\noindent\textit{Note.} Degree < 4 is exactly 'touches the edge'. O(R*C); the closed form 2R + 2C - 4 (clamped) is the optimisation to mention, not to start with.
...
def count_islands(grid):
    ...
    return n
\end{lstlisting}
\noindent\textit{Note.} Outer scan + inner flood: every cell enters the queue at most once, O(R*C). Mark seen when ENQUEUING, not when popping, or the queue balloons with duplicates.
...
\end{lstlisting}
\noindent\textit{Note.} distinct moves only when a count crosses 0<->1 -- that boundary bookkeeping is the whole problem. k = 0 forces the window empty forever: answer 0. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def longest_within_budget(xs, budget):
    total = [0]
    def add(x): total[0] += x
    def rem(x): total[0] -= x
    return longest_window(xs, lambda: total[0] <= budget, add, rem)
\end{lstlisting}
\noindent\textit{Note.} Shrinking is only sound because durations are non-negative (removing can never make the sum worse) -- say that invariant out loud; with negatives this window logic breaks.
...
\end{lstlisting}
\noindent\textit{Note.} Each land cell contributes 4 edges minus one per land neighbour; the bounds-checked generator makes border edges fall out for free. O(R*C), no flood needed.
...
\end{lstlisting}
\noindent\textit{Note.} BFS eccentricity: the dist map doubles as the seen-set, and the answer is its maximum rather than a target lookup -- the same machine as shortest-path, read differently.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def max_up_with_repairs(xs, k):
    zeros = [0]
    def add(x): zeros[0] += x == 0
    def rem(x): zeros[0] -= x == 0
    return longest_window(xs, lambda: zeros[0] <= k, add, rem)
\end{lstlisting}
\noindent\textit{Note.} LeetCode 1004 in window form: track zeros in the window, legal while <= k. The insight worth voicing: you never 'use' repairs -- the window just tolerates k zeros. O(n).
...
\end{lstlisting}
\noindent\textit{Note.} Multi-source BFS is ordinary BFS with several starting points enqueued at time 0; the distance of the last fresh orange is the answer. Count fresh up front so unreachable ones are detectable -- the classic miss is returning minutes without that check.
...
\end{lstlisting}
\noindent\textit{Note.} Direct recurrence plus memo; O(n) states, O(1) work each. Without memo this is O(3\^{}n) -- the whole lesson in one decorator.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
first_pow2 = lambda n: until(lambda x: x >= n, lambda x: 2 * x, 1)
\end{lstlisting}
\noindent\textit{Note.} until(stop, step, seed) is a while loop as an expression. O(log n) doublings.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
digital_root = lambda n: until(lambda x: x < 10, lambda x: sum(map(int, str(x))), n)
\end{lstlisting}
\noindent\textit{Note.} Stop predicate x < 10; the step is one digit-sum. Each step shrinks the number fast, so only a handful of iterations ever run.
...
\end{lstlisting}
\noindent\textit{Note.} The two base cases ARE the specification: one way to be done, no way to overshoot. O(n) states.
...
\end{lstlisting}
\noindent\textit{Note.} memo exposes .cache precisely so the table is inspectable: n+1 states for fib(n). Defining the helper inside gives every call a fresh table.
...
def rob(xs):
    ...
    return go(0)
\end{lstlisting}
\noindent\textit{Note.} State: index; choice: take (jump 2) or skip (jump 1). O(n) states. The classic mistake is greedy on the largest house -- [2,7,9] breaks it.
...
def decode_ways(s):
    ...
    return go(0)
\end{lstlisting}
\noindent\textit{Note.} State: position; two branches (one digit, two digits) guarded by the 10..26 window and the zero rule. O(n) states; the '0' guard is where most attempts die.
...
\end{lstlisting}
\noindent\textit{Note.} Greedy reach as a fold; -1 is the poison value that propagates (the g9 trick on a number). O(n), no DP table needed -- reach is monotone.
...
def num_squares(n):
    ...
    return go(n)
\end{lstlisting}
\noindent\textit{Note.} Coin change with square coins; isqrt(m) trims the branch factor. O(n * sqrt(n)). Greedy (largest square first) fails on 12.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
def years_to_target(pop, rate, influx, target):
    grow = lambda s: (s[0] * (100 + rate) // 100 + influx, s[1] + 1)
    return snd(until(lambda s: s[0] >= target, grow, (pop, 0)))
\end{lstlisting}
\noindent\textit{Note.} The tuple state carries the counter alongside the value -- until's standard idiom. Integer arithmetic keeps the doctests exact.
...
\end{lstlisting}
\noindent\textit{Note.} The lcs skeleton on one string against itself, expressed as a shrinking window. O(n\^{}2) states; the i == j base (a single char is a palindrome) is the easy one to forget.
...
def word_break(s, words):
    ...
    return go(0)
\end{lstlisting}
\noindent\textit{Note.} State: start index -- the future only needs where the unbroken suffix begins. any() short-circuits; memo caps the work at O(len(s) * len(words)) prefix checks.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
clean_reverse = compose(lambda s: s[::-1], str.lower, str.strip)
\end{lstlisting}
\noindent\textit{Note.} compose(f, g, h)(x) == f(g(h(x))): strip happens first because it is rightmost. Point-free: no lambda parameter of your own.

\subsection*{16.2\quad Flip subtract\hfill\textnormal{\textit{drill}}}
The prelude's sub(a, b) = a - b. Use flip to build a function that subtracts the FIRST argument from the second. flip swaps argument order and nothing else.
...
rsub = flip(sub)
\end{lstlisting}
\noindent\textit{Note.} flip is a pure adapter -- it computes nothing, just reorders. This is exactly how the prelude derives foldr from foldl.

\subsection*{16.3\quad Add via curry\hfill\textnormal{\textit{drill}}}
The prelude's add(a, b) = a + b. Curry it, then use the curried form to build add\_ten, a one-argument function that adds 10. Show that binding the first argument yields a reusable function.
...
add_ten = curry(add)(10)
\end{lstlisting}
\noindent\textit{Note.} curry(add)(10) captures 10 in a closure and returns a function still waiting for the second argument. That half-applied function is a value you can name.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
mask = lambda xs: map_(const('x'), xs)
\end{lstlisting}
\noindent\textit{Note.} const fills a callback slot with a constant. Reaching for a lambda z: 'x' would work too, but const names the intent.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
powers_of_two = partial(pow, 2)
\end{lstlisting}
\noindent\textit{Note.} partial binds any number of leading args in one shot; curry binds them one call at a time. Here one frozen base is all we need.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
by_age_desc = lambda people: sortBy(on(flip(sub), snd), people)
\end{lstlisting}
\noindent\textit{Note.} on separates the projection (snd) from the comparison (flip(sub), b - a, for descending). This is Haskell's 'comparing' idiom; a per-element key with sortOn would also work here.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
RULES = [(lambda n: n < 0, const('neg')),
         (lambda n: n == 0, const('zero')),
         (const(otherwise), const('pos'))]
classify = lambda n: snd(find(lambda rule: fst(rule)(n), RULES))(n)
\end{lstlisting}
\noindent\textit{Note.} An if/elif ladder rewritten as data: rules become appendable and testable in isolation. const(otherwise) is the always-true arm; a bare True would crash when applied.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
pipe = lambda steps, x: compose(*reversed(steps))(x)
\end{lstlisting}
\noindent\textit{Note.} compose is right-to-left; a left-to-right pipeline just reverses the list first. The empty pipeline is the identity, which compose() already gives you.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
pair = lambda w: (len(w), 1)
length_histogram = lambda words: fromListWith(add, map_(pair, words))
\end{lstlisting}
\noindent\textit{Note.} fromListWith is the dict-building fold; addition is commutative so the new/old order is irrelevant here. O(n).
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
same_by = lambda key, a, b: on(lambda x, y: x == y, key)(a, b)
\end{lstlisting}
\noindent\textit{Note.} on(eq, key) is the reusable 'equal through a projection'. partial(same\_by, str.lower) then feeds groupBy a two-argument equivalence -- combinators composing with the containers.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
first_error = lambda validators, x: find(lambda e: e is not None,
                                          map_(lambda v: v(x), validators))
\end{lstlisting}
\noindent\textit{Note.} map\_ runs every validator, find returns the first non-None (the first failure), else None. The None-is-Nothing convention makes 'no error' and 'passed' the same value.
...
\end{lstlisting}
\noindent\textit{Note.} get\_in returns a closure over keys -- currying by hand -- so a path becomes a reusable accessor. The fold threads the dict through each key, poisoning to None on any miss.

\clearpage
\chapter{Monoids and Monads}
...
these instead.
\section{Concepts}
\subsection{\texttt{Monoid}}\label{def:Monoid}
reifies "an associative combiner with an identity" as a plain pair (mempty, mappend). Naming the pair is the whole trick: once a monoid is a VALUE, one fold engine (mconcat) serves every instance, and instances can be combined (both) or invented on the spot. The two laws you must supply: op is associative, and empty is its do-nothing element.
\begin{lstlisting}[style=ex]
>>> Votes = Monoid(0, add)
...
\end{lstlisting}
\subsection{\texttt{mconcat}}\label{def:mconcat}
collapses a whole list with a monoid: foldl of the pair's op, seeded by the pair's empty. The empty seed is what makes the empty list a lawful answer instead of an error.
\begin{lstlisting}[style=ex]
>>> mconcat(Sum, [1, 2, 3])
6
>>> mconcat(Sum, [])
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{ListM} in \S\ref{def:ListM} (p.~\pageref{def:ListM}), \texttt{Sum} in \S\ref{def:Sum} (p.~\pageref{def:Sum}).}

\subsection{\texttt{foldMap}}\label{def:foldMap}
map each element into the monoid, then mconcat: the "measure and combine" one-liner. Most monoid uses are foldMap uses -- the elements are rarely already the thing you want to combine.
\begin{lstlisting}[style=ex]
...
Sum, Product, All, Any -- the numeric and boolean instances. Each is just (identity, operator): what sum/product/all/any hard-code, written as data.
...
MinM, MaxM -- min and max with honest identities: inf loses every min, -inf loses every max, so the empty list has an answer instead of an exception.
...
First, Last -- "first non-None wins" and "last non-None wins": fallback chains as folds. Config lookup in one line.
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{All} in \S\ref{def:All} (p.~\pageref{def:All}), \texttt{Any} in \S\ref{def:Any} (p.~\pageref{def:Any}), \texttt{First} in \S\ref{def:First} (p.~\pageref{def:First}), \texttt{Last} in \S\ref{def:Last} (p.~\pageref{def:Last}), \texttt{ListM} in \S\ref{def:ListM} (p.~\pageref{def:ListM}), \texttt{MaxM} in \S\ref{def:MaxM} (p.~\pageref{def:MaxM}), \texttt{MinM} in \S\ref{def:MinM} (p.~\pageref{def:MinM}), \texttt{Product} in \S\ref{def:Product} (p.~\pageref{def:Product}), \texttt{Sum} in \S\ref{def:Sum} (p.~\pageref{def:Sum}).}

\textbf{\texttt{All}}\label{def:All} --- (see prelude.py)

\begin{lstlisting}[style=ex]
All = Monoid(True, lambda a, b: a and b)  # mconcat(All, bs)  == all(bs)
\end{lstlisting}
\textbf{\texttt{Any}}\label{def:Any} --- (see prelude.py)

\begin{lstlisting}[style=ex]
Any = Monoid(False, lambda a, b: a or b)  # mconcat(Any, bs)  == any(bs)
\end{lstlisting}
\textbf{\texttt{First}}\label{def:First} --- (see prelude.py)

\begin{lstlisting}[style=ex]
First = Monoid(None, lambda a, b: b if a is None else a)  # first non-None wins
\end{lstlisting}
\textbf{\texttt{Last}}\label{def:Last} --- (see prelude.py)

\begin{lstlisting}[style=ex]
Last = Monoid(None, lambda a, b: a if b is None else b)  # last non-None wins
\end{lstlisting}
\textbf{\texttt{ListM}}\label{def:ListM} --- (see prelude.py)

\begin{lstlisting}[style=ex]
ListM = Monoid([], add)  # concat as a monoid
\end{lstlisting}
\textbf{\texttt{MaxM}}\label{def:MaxM} --- (see prelude.py)

\begin{lstlisting}[style=ex]
MaxM = Monoid(float("-inf"), max)  # identity -inf: max of nothing loses to all
\end{lstlisting}
\textbf{\texttt{MinM}}\label{def:MinM} --- (see prelude.py)

\begin{lstlisting}[style=ex]
MinM = Monoid(float("inf"), min)  # identity inf: min of nothing loses to all
\end{lstlisting}
\textbf{\texttt{Product}}\label{def:Product} --- (see prelude.py)

\begin{lstlisting}[style=ex]
Product = Monoid(1, mul)  # mconcat(Product, xs) == product(xs)
\end{lstlisting}
\textbf{\texttt{Sum}}\label{def:Sum} --- (see prelude.py)

\begin{lstlisting}[style=ex]
Sum = Monoid(0, add)  # mconcat(Sum, xs)     == sum(xs)
\end{lstlisting}
\subsection{\texttt{both}}\label{def:both}
the tuple monoid: pairs two monoids into one, so TWO statistics fall out of ONE pass. Chain it for three or more.
\begin{lstlisting}[style=ex]
...
\end{lstlisting}
\subsection{\texttt{bind}}\label{def:bind}
the Maybe monad's >>=: feed x to f, unless x is already None -- then stay None. Failure propagates itself; the if-ladder disappears. The standing caveat of None-as-Nothing applies: a step cannot return None as a SUCCESS.
\begin{lstlisting}[style=ex]
>>> bind(4, succ)
5
...
\end{lstlisting}
\subsection{\texttt{chainM}}\label{def:chainM}
Kleisli composition (>=>): glue failable steps into one failable pipeline. Unlike compose, the LEFTMOST function runs first -- data flows left to right, reading order.
\begin{lstlisting}[style=ex]
...
\end{lstlisting}
\subsection{\texttt{sequenceM}}\label{def:sequenceM}
[Maybe a] -> Maybe [a]: all-or-nothing. Every element present, the list comes back; one None anywhere, the WHOLE answer is None.
\begin{lstlisting}[style=ex]
>>> sequenceM([1, 2, 3])
[1, 2, 3]
...
\end{lstlisting}
\subsection{\texttt{traverseM}}\label{def:traverseM}
map a failable function, then demand every success: sequenceM after map\_. The batch-parse shape -- contrast mapMaybe, which silently DROPS failures; traverseM refuses the whole batch.
\begin{lstlisting}[style=ex]
...
Ok, Err -- Either's constructors as tagged pairs: ("ok", value) or ("err", why). A Maybe that keeps the reason for the failure.
...
\end{lstlisting}
\noindent{\small\itshape Used before it is rolled: \texttt{Ok} in \S\ref{def:Ok} (p.~\pageref{def:Ok}), \texttt{Err} in \S\ref{def:Err} (p.~\pageref{def:Err}).}

\textbf{\texttt{Ok}}\label{def:Ok} --- (see prelude.py)

\begin{lstlisting}[style=ex]
Ok, Err = lambda v: ("ok", v), lambda e: ("err", e)  # tagged pairs; hoisted ahead of chainE
\end{lstlisting}
\textbf{\texttt{Err}}\label{def:Err} --- (see prelude.py)

\begin{lstlisting}[style=ex]
Ok, Err = lambda v: ("ok", v), lambda e: ("err", e)  # tagged pairs; hoisted ahead of chainE
\end{lstlisting}
\subsection{\texttt{bindE}}\label{def:bindE}
Either's >>=: pass the value onward, or short-circuit carrying the message. The first error wins and survives to the end.
\begin{lstlisting}[style=ex]
...
\end{lstlisting}
\subsection{\texttt{chainE}}\label{def:chainE}
Kleisli composition for Either: wraps the input in Ok, then threads it through Ok/Err-returning steps. Same shape as chainM, plus the WHY.
\begin{lstlisting}[style=ex]
...
challenges are interview-grade. Write each solution in a scratch file with
the given doctests pasted in, and let \texttt{python3 -m doctest} referee.
\subsection*{17.1\quad An XOR monoid\hfill\textnormal{\textit{drill}}}
Build a Monoid pairing False (the identity) with XOR as the combining op, then use mconcat to fold a whole list of bools through it. XOR toggles a bit once per True.

\noindent\texttt{xor\_all(bits: list[bool]) -> bool}
\begin{lstlisting}[style=ex]
>>> xor_all([True, False, True])
False
>>> xor_all([True])
True
>>> xor_all([True, True])
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
XorM = Monoid(False, lambda a, b: a != b)
xor_all = lambda bits: mconcat(XorM, bits)
\end{lstlisting}
\noindent\textit{Note.} Naming the pair is the whole trick: once XOR is a monoid, mconcat is the fold engine for free. The identity False is why the empty list answers False, not an error.

\subsection*{17.2\quad Total length via foldMap\hfill\textnormal{\textit{drill}}}
Use foldMap with the Sum monoid to add up the lengths of a list of strings. foldMap maps each element into the monoid (here, its length) and then mconcats the results.

\noindent\texttt{total\_length(words: list[str]) -> int}
\begin{lstlisting}[style=ex]
>>> total_length(['ab', 'c', ''])
3
>>> total_length([])
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
total_length = lambda words: foldMap(len, Sum, words)
\end{lstlisting}
\noindent\textit{Note.} foldMap(f, m, xs) is 'measure, then combine': len is the measure, Sum's (0, add) is the combiner. Most monoid use in practice is foldMap use, not a bare mconcat.

\subsection*{17.3\quad Factorial via Product\hfill\textnormal{\textit{drill}}}
Use mconcat with the Product monoid over range(1, n + 1) to compute n!. Product's identity is 1, so the empty product (n == 0) comes out right with no special case.
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
factorial = lambda n: mconcat(Product, range(1, n + 1))
\end{lstlisting}
\noindent\textit{Note.} Product's identity 1 is what makes the n == 0 case (an empty range) a lawful factorial instead of a special-cased branch -- that is the whole point of an identity element.

\subsection*{17.4\quad Max and min in one pass\hfill\textnormal{\textit{apply}}}
Use both(MaxM, MinM) to build a monoid that tracks the running maximum AND minimum together, then mconcat a list of (x, x) pairs through it -- one pass, two answers.

\noindent\texttt{max\_min(xs: list[int]) -> tuple[int, int]}
\begin{lstlisting}[style=ex]
>>> max_min([3, 1, 4, 1, 5])
(5, 1)
>>> max_min([7])
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
MaxMin = both(MaxM, MinM)
max_min = lambda xs: mconcat(MaxMin, [(x, x) for x in xs])
\end{lstlisting}
\noindent\textit{Note.} both pairs two monoids into one over pairs: the tuple monoid's op runs each side's op on its own half. Chain both again for a third statistic in the same single pass.

\subsection*{17.5\quad Safe square-root-then-reciprocal\hfill\textnormal{\textit{apply}}}
Chain two None-propagating steps with chainM: first take a square root (only for n >= 0), then a reciprocal (only if the result is nonzero). chainM runs its LEFTMOST function first, and bind stops the whole chain the moment a step returns None.

\noindent\texttt{safe\_pipeline(n: float) -> float | None}
\begin{lstlisting}[style=ex]
>>> safe_pipeline(4)
0.5
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
sqrt_step = lambda n: n ** 0.5 if n >= 0 else None
recip_step = lambda n: 1 / n if n != 0 else None
safe_pipeline = chainM(sqrt_step, recip_step)
\end{lstlisting}
\noindent\textit{Note.} chainM(*fs) folds bind over the steps left to right -- the OPPOSITE order from compose. None from either step poisons everything after it without an if-ladder.

\subsection*{17.6\quad Parse every entry, or fail entirely\hfill\textnormal{\textit{apply}}}
Use traverseM to parse a list of strings as integers, demanding EVERY entry succeed. A single unparseable entry should make the whole result None -- all-or-nothing, like sequenceM but with the parse step folded in.

\noindent\texttt{parse\_all(strs: list[str]) -> list[int] | None}
\begin{lstlisting}[style=ex]
>>> parse_all(['1', '2', '3'])
[1, 2, 3]
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
parse_one = lambda s: int(s) if s.lstrip('-').isdigit() else None
parse_all = lambda strs: traverseM(parse_one, strs)
\end{lstlisting}
\noindent\textit{Note.} traverseM(f, xs) is sequenceM(map\_(f, xs)) fused into one call: map a failable step, then demand every success. The empty list traverses to [], vacuously all-or-nothing.

\subsection*{17.7\quad Validate an age, keeping the reason\hfill\textnormal{\textit{challenge}}}
Where Maybe only says 'it failed', Either says WHY. Write two Ok/Err-returning steps -- parse a digit string, then range-check it -- and chain them with chainE. chainE's first step still takes the raw value; only the WIRING between steps is Ok/Err-aware.

\noindent\texttt{validate\_age(raw: str) -> tuple}
\begin{lstlisting}[style=ex]
>>> validate_age('42')
('ok', 42)
>>> validate_age('abc')
...
\noindent\textbf{Solution.}
\begin{lstlisting}[style=code]
parse_step = lambda s: Ok(int(s)) if s.isdigit() else Err('not a number')
range_step = lambda n: Ok(n) if 0 <= n <= 150 else Err('out of range')
validate_age = chainE(parse_step, range_step)
\end{lstlisting}
\noindent\textit{Note.} bindE short-circuits on the first Err but keeps its message, unlike bind's bare None. chainE(*fs) is exactly chainM's shape, generalised from 'failed or not' to 'failed, why'.

\subsection*{17.8\quad Validate a whole roster\hfill\textnormal{\textit{challenge}}}
Hand-roll a sequenceE by folding bindE over a list, reusing a per-entry validator like 17.7's: collect every valid age into one Ok list, or stop at the first Err and report it. The accumulator threads an Either the way sequenceM threads a Maybe.

\noindent\texttt{validate\_roster(raws: list[str]) -> tuple}
\begin{lstlisting}[style=ex]
>>> validate_roster(['10', '20', '30'])
('ok', [10, 20, 30])
...
\end{lstlisting}
\noindent\textit{Note.} Ok([]) seeds the fold the way [] seeds sequenceM; each step's bindE only advances the accumulator when both the prior entries and this one are Ok. First Err wins and sticks.
...
contracts, hints and doctests are printed here; solutions follow each problem in this edition.
